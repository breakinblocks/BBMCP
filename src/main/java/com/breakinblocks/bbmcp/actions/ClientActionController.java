...
public final class ClientActionController implements AutoCloseable {
    private static final int MAX_COMPLETED_STATUSES = 256;
    private final Minecraft minecraft;
    private final AtomicLong nextId = new AtomicLong();
    private final Map<Long, ActionStatus> statuses = new java.util.HashMap<>();
    private final ArrayDeque<Long> completedStatusIds = new ArrayDeque<>();
    ...
    private void finish(long id, ActionStatus.State state, String message) {
        ...
        if (state != ActionStatus.State.STARTED) {
            completedStatusIds.addLast(id);
            while (completedStatusIds.size() > MAX_COMPLETED_STATUSES) {
                statuses.remove(completedStatusIds.removeFirst());
            }
        }
        if (activeAction != null && activeId(activeAction) == id) {
            activeAction = null;
    ...
    private static void requireTicks(int ticks) {
        if (ticks < 1 || ticks > BbmcpConfig.maxActionTicks()) {
            throw new IllegalArgumentException(
                    "Ticks must be between 1 and " + BbmcpConfig.maxActionTicks());
        }
    }
