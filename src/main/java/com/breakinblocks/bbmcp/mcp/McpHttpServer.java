...
public final class McpHttpServer implements AutoCloseable {
    ...
    public synchronized void start() throws IOException {
        ...
        int port = BbmcpConfig.port();
        int requestThreadCount = BbmcpConfig.requestThreadCount();
        int requestQueueCapacity = BbmcpConfig.requestQueueCapacity();
        int sseConnectionLimit = BbmcpConfig.sseConnectionLimit();
        int sseQueueCapacity = BbmcpConfig.sseQueueCapacity();
        ...
        ExecutorService newSseExecutor = new ThreadPoolExecutor(
                ...
                namedDaemonThreadFactory("BBMCP-sse-"),
                new ThreadPoolExecutor.AbortPolicy());
        try {
            ...
            synchronized (McpHttpServer.class) {
                if (activeServer != null) {
                    ...
                    sseExecutor = null;
                    throw new IllegalStateException("Another BBMCP HTTP server is already active");
                }
                activeServer = this;
            }
            server = newServer;
    ...
    private boolean handleEventStream(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            client = new SseClient(exchange.getResponseBody(), BbmcpConfig.sseQueueCapacity());
        ...
        try {
            ...
            while (!client.isClosed()) {
                String message = client.awaitMessage(
                        BbmcpConfig.sseHeartbeatSeconds(),
                        TimeUnit.SECONDS);
                if (message == null) {
    ...
    private boolean tryReserveSseConnection() {
        while (true) {
            ...
            if (current >= BbmcpConfig.sseConnectionLimit()) {
                return false;
            }
    ...
    private String readRequestBody(InputStream input) throws IOException, RequestTooLargeException {
        ...
        while ((bytesRead = input.read(buffer)) != -1) {
            if (bytesRead > BbmcpConfig.maxRequestBytes() - totalBytes) {
    ...
    private JsonObject initialize() {
        ...
        serverInfo.addProperty("name", "BBMCP");
    ...
    private JsonObject toolsList() {
        ...
        if (BbmcpConfig.allowCommandExecution()) {
            tools.add(command);
        }
        ...
        tools.add(tool(
                "read_latest_logs",
                "Return the last " + BbmcpConfig.maxLogLines()
                        + " lines of the active Minecraft logs/latest.log file."));
        if (BbmcpConfig.allowKubejsScriptInjection()) {
            tools.add(tool(
    ...
    private JsonObject executeCommand(JsonObject arguments) throws Exception {
        ...
        if (!BbmcpConfig.allowCommandExecution()) {
            throw new InvalidParamsException("execute_command is disabled by BBMCP configuration");
        }
        if (command.isBlank()) {
        ...
        if (command.length() > BbmcpConfig.maxCommandLength()) {
            throw new InvalidParamsException(
                    "execute_command command exceeds the configured maximum length of "
                            + BbmcpConfig.maxCommandLength());
        }
    ...
    private JsonObject getNearbyEntities(JsonObject arguments) throws Exception {
        ...
        if (radius < 0.0D || radius > BbmcpConfig.maxNearbyEntityRadius()) {
            throw new InvalidParamsException(
                    "get_nearby_entities radius must be between 0 and "
                            + BbmcpConfig.maxNearbyEntityRadius());
        }
        try {
    ...
    private JsonObject findRecipes(JsonObject arguments) throws Exception {
        ...
        int limit = optionalInteger(arguments, "limit", "find_recipes", BbmcpConfig.maxRecipeResults());
        if (limit < 1 || limit > BbmcpConfig.maxRecipeResults()) {
            throw new InvalidParamsException(
                    "find_recipes limit must be between 1 and " + BbmcpConfig.maxRecipeResults());
        }
        try {
    ...
    private JsonObject getRecipeTree(JsonObject arguments) throws Exception {
        ...
        int maxDepth = optionalInteger(arguments, "max_depth", "get_recipe_tree", BbmcpConfig.maxRecipeTreeDepth());
    ...
    private JsonObject scanForLoops(JsonObject arguments) throws Exception {
        ...
        int maxDepth = optionalInteger(arguments, "max_depth", "scan_for_loops", BbmcpConfig.maxRecipeLoopDepth());
        if (maxDepth < 1 || maxDepth > BbmcpConfig.maxRecipeLoopDepth()) {
            throw new InvalidParamsException(
                    "scan_for_loops max_depth must be between 1 and " + BbmcpConfig.maxRecipeLoopDepth());
        }
        try {
    ...
    private JsonObject recipeItemDepthTool(String name, String description) {
        ...
        if ("get_recipe_tree".equals(name)) {
            properties.add("max_depth", integerSchema(0, BbmcpConfig.maxRecipeTreeDepth(),
                    BbmcpConfig.maxRecipeTreeDepth()));
        } else if ("scan_for_loops".equals(name)) {
            properties.add("max_depth", integerSchema(1, BbmcpConfig.maxRecipeLoopDepth(),
                    BbmcpConfig.maxRecipeLoopDepth()));
        } else {
    ...
    private void requireRecipeDepth(int depth, String toolName) throws InvalidParamsException {
        if (depth < 0 || depth > BbmcpConfig.maxRecipeTreeDepth()) {
            throw new InvalidParamsException(
                    toolName + " max_depth must be between 0 and " + BbmcpConfig.maxRecipeTreeDepth());
        }
    }
    ...
    private void requireActionTicks(int ticks, String toolName) throws InvalidParamsException {
        if (ticks < 1 || ticks > BbmcpConfig.maxActionTicks()) {
            throw new InvalidParamsException(
                    toolName + " duration_ticks must be between 1 and " + BbmcpConfig.maxActionTicks());
        }
    }
    ...
    private ThreadFactory namedDaemonThreadFactory() {
        return namedDaemonThreadFactory("BBMCP-http-");
    }
