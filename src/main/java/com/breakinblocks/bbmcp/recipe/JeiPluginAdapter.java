...
public final class JeiPluginAdapter implements IRecipeViewerAdapter {
    ...
    private void enrichWorkstations(IJeiRuntime runtime, JsonObject itemNode) {
        ...
        for (int index = 0; index < recipes.size(); index++) {
            ...
            try {
                ...
                recipe.addProperty("workstation_metadata_available", true);
            } catch (UnsupportedWorkstationMetadataException exception) {
                recipe.add("required_workstations", new JsonArray());
                recipe.addProperty("workstation_metadata_available", false);
    ...
    private JsonArray workstationIds(IJeiRuntime runtime, Recipe<?> recipe) {
        ...
        RecipeType<?> jeiType = runtime.getJeiHelpers().getRecipeType(typeId)
                .orElseThrow(() -> new UnsupportedWorkstationMetadataException(
                        "JEI recipe category is unavailable: " + typeId));
    ...
    private static final class UnsupportedWorkstationMetadataException extends RuntimeException {
        private UnsupportedWorkstationMetadataException(String message) {
            super(message);
        }
    }
    ...
    private <T> void renderLayout(
        ...
        IRecipeLayoutDrawable<T> layout = manager.createRecipeLayoutDrawable(
                        category,
                        recipe,
                        BbmcpJeiPlugin.runtime().getJeiHelpers().getFocusFactory().getEmptyFocusGroup())
